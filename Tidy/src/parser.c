...
/*
  XML documents
*/
static void ParseXMLElement(TidyDocImpl* doc, Node *element, GetTokenMode mode, int recursionDepth)
{
	//Fixing buffer overruns - P.K.
	int maxDepth=100;
    ...
    while ((node = TY_(GetToken)(doc, mode)) != NULL)
    {
        ...
        if (node->type == StartTag && recursionDepth < maxDepth)
            ParseXMLElement( doc, node, mode, recursionDepth+1 );

		//Recursion Depth overrun - P.K.
		if( recursionDepth >= maxDepth )
		{
			//Let's report it as an error
			TY_(ReportFatal)(doc, element, node, UNEXPECTED_ENDTAG);
...
void TY_(ParseXMLDocument)(TidyDocImpl* doc)
{
    ...
    while ((node = TY_(GetToken)(doc, IgnoreWhitespace)) != NULL)
    {
       ...
       /* if start tag then parse element's content */
        ...
        {
            TY_(InsertNodeAtEnd)( &doc->root, node );
            ParseXMLElement( doc, node, IgnoreWhitespace, 0 );
            continue;
        }
